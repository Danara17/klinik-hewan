...
@section('content')
    ...
    <div class="overflow-x-auto dark:text-white">
        <table id="myTable" class="table-auto min-w-full dark:bg-gray-800">
            ...
            <tbody>
                @foreach ($master_pet_type as $item)
                    <tr class="dark:bg-gray-700">
                        <td class="">{{ $loop->iteration }}</td>
                        <td class="">{{ $item->name }}</td>
                        <td class="gap-2">
                            <a href="{{ route('pet_type.edit', $item->id) }}"
                            ...
                            <form action="{{ route('pet_type.destroy', $item->id) }}" method="POST"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                    <svg class="w-3 h-3 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                        width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                        <path fill-rule="evenodd"
