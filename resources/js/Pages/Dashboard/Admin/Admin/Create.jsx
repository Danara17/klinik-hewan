...
function Create({ name, email, role, avatar, page_title, dataUser,  }) {
    ...
    const handleSubmit = (event) => {
        ...
        router.post('/dashboard/admin', data)
    }
    ...
    return (
        <Dashboard name={name} email={email} role={role} avatar={avatar} page_title={page_title} >
            ...
            <form onSubmit={handleSubmit} >
                <div className="mb-3">
                    <label htmlFor="id_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Pilih User
                    </label>
                    <select
                        id="id_user"
                        name="id_name"
                        value={data.id_name}
                        onChange={(e) => setData('id_name', e.target.value)}
