...
export default function FaqContent() {
    ...
    const faqs = [
        {
            question:
                "Jenis layanan perawatan hewan apa saja yang Anda tawarkan?",
        ...
        <div className="py-28">
            <div className="mx-auto w-full max-w-screen-xl px-4">
                <div className="flex flex-col md:flex-row gap-20 ">
                    <div
                        className=" rounded-lg flex flex-col gap-1 p-7"
                        data-aos="fade-right"
                    >
                        <h1 className="text-3xl font-bold text-center mt-[-18px] mb-4">
                            Frequently Asked Questions ?<p>(FAQ)</p>
                        </h1>
                        {faqs.map((faq, index) => (
                            <div
                                key={index}
                                className={`border rounded-lg shadow-lg w-full md:w-[28rem] ${
                                    index === faqs.length - 1 ? "mb-0" : "mb-5"
                                }`}
                            >
                                <button
                                    className="text-lg font-semibold py-2 px-4 bg-gray-200 rounded-lg w-full text-left flex justify-between items-center"
